/* NEXO UI kit — Icon component (lucide glyphs, inlined) + brand primitives.
   Stroke 2, rounded — matches the product's lucide-react usage. */
...
function Icon({ name, size = 18, color = "currentColor", strokeWidth = 2, className = "", style = {} }) {
  return (
    <svg
      width={size} height={size} viewBox="0 0 24 24" fill="none"
      stroke={color} strokeWidth={strokeWidth} strokeLinecap="round" strokeLinejoin="round"
      className={className} style={style}
      dangerouslySetInnerHTML={{ __html: NX_ICONS[name] || "" }}
    />
  );
}
...
function Brand({ onClick }) {
  return (
    <span className="brand" onClick={onClick}>
      <span className="brand-mark" aria-hidden="true"><Icon name="network" size={19} strokeWidth={2.1} /></span>
      <span>NEXO</span>
    </span>
  );
}

Object.assign(window, { Icon, Brand, NX_ICONS });
